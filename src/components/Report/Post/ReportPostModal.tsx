...
function ReportPostModal() {
  ...
  const data = contentData?.data.data.posts[modalStatus.idx];
  ...
  const {
    ...
    queryFn: async ({ queryKey, pageParam = 0 }) => {
      ...
      //postId로 수정해주세용,
      const res = await getReportedPostDetailRequest(+postId, pageParam);
              ...
              <section className="flex-grow basis-1/2 h-full overflow-auto p-2">
                ...
                <div className="border my-4">
                  ...
                  {reportedList?.pages.map((page) =>
                    page.result.postReports.map((report) => (
                      <div
...
export const useReportPostModal = () => {
  return useGlobalModal('report-post');
};
